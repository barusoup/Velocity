...
## Don't
- Don't simplify or "DRY up" the publish-job `TAG` derivation in
  `.github/workflows/release.yml`. Two non-obvious things are load-bearing:
  the input check captures `VERSION="${{ github.event.inputs.version }}"`
  and guards with `[ "$VERSION" != "null" ]` to dodge the GitHub-Actions
  rendering trap (an undefined `github.event.inputs.X` is rendered as
  the literal string `"null"` on tag-push events, which would build a
  nonsensical `refs/tags/vnull:refs/tags/vnull` refspec); the tag fetch
  uses `git fetch --force origin "refs/tags/${TAG}:refs/tags/${TAG}"`
  because `actions/checkout@v4` writes the local `<TAG>` ref as the
  peeled commit object rather than the original annotated-tag object —
  Git 2.20+ refuses the overwrite without `--force` and the publish
  job fails at `Fetch annotated tag for release notes` with
  `! [rejected] (would clobber existing tag)`. Symptom in run history:
  hash alone, no vX.Y.Z record. If a "cleanup" PR drops either
  piece, every subsequent release silently regresses — re-push the
  tag after merging to verify the publish job passes.
